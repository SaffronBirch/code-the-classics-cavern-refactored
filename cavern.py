...
# App class that acts as the context, maintaining state changes
class App:
    def __init__(self):
...
class MenuScreen(GameState):
    def update(self, input_state):
        if input_state.fire_pressed:
            ...
            self.app.change_state(PlayScreen(self.app))
            self.app.game = Game(Player())
        else:
            self.app.game.update(input_state)

    def draw(self):
        self.app.game.draw()

        screen.blit("title", (0, 0))
        anim_frame = min(((self.app.game.timer + 40) % 160) // 4, 9)
...
class PlayScreen(GameState):
    ...
    def update(self, input_state):
        ...
        if self.app.game.player.lives < 0:
            ...
            self.app.change_state(GameOverScreen(self.app))
        else:
            self.app.game.update(input_state)
...
class GameOverScreen(GameState):
    def update(self, input_state):
        if input_state.fire_pressed:
            ...
            self.app.change_state(MenuScreen(self.app))
            self.app.game = Game()
...
class Orb(CollideActor):
    ...
    def update(self):
        ...
        if self.timer == self.blown_frames:
            ...
            app.game.pops.append(Pop(self.pos, 1))
            if self.trapped_enemy_type != None:
                ...
                app.game.fruits.append(Fruit(self.pos, self.trapped_enemy_type))
            app.game.play_sound("pop", 4)
...
class Bolt(CollideActor):
    ...
    def update(self):
        ...
        if self.move(self.direction_x, 0, Bolt.SPEED):
            ...
            for obj in app.game.orbs + [app.game.player]:
                if obj and obj.hit_test(self):
                    self.active = False
                    break

        direction_idx = "1" if self.direction_x > 0 else "0"
        anim_frame = str((app.game.timer // 4) % 2)
...
class Fruit(GravityActor):
    ...
    def update(self):
        ...
        if app.game.player and app.game.player.collidepoint(self.center):
            if self.type == Fruit.EXTRA_HEALTH:
                app.game.player.health = min(3, app.game.player.health + 1)
                app.game.play_sound("bonus")
            elif self.type == Fruit.EXTRA_LIFE:
                app.game.player.lives += 1
                app.game.play_sound("bonus")
            else:
                app.game.player.score += (self.type + 1) * 100
                app.game.play_sound("score")
        ...
        if self.time_to_live <= 0:
            ...
            app.game.pops.append(Pop((self.x, self.y - 27), 0))

        anim_frame = str([0, 1, 2, 1][(app.game.timer // 6) % 4])
...
class Player(GravityActor):
    ...
    def hit_test(self, other):
        ...
        if self.collidepoint(other.pos) and self.hurt_timer < 0:
            ...
            if self.health > 0:
                app.game.play_sound("ouch", 4)
            else:
                app.game.play_sound("die")
            return True
        else:
            return False
    ...
    def update(self, input_state):
        ...
        if self.hurt_timer > 100:
            ...
            if input_state.fire_pressed and self.fire_timer <= 0 and len(app.game.orbs) < 5:
                ...
                app.game.orbs.append(self.blowing_orb)
                app.game.play_sound("blow", 4)
                self.fire_timer = 20
        ...
        if self.hurt_timer <= 0 or self.hurt_timer % 2 == 1:
            ...
            if self.hurt_timer > 100:
                if self.health > 0:
                    ...
                    self.image = "fall" + str((app.game.timer // 4) % 2)
            elif self.fire_timer > 0:
                self.image = "blow" + dir_index
            elif dx == 0:
                self.image = "still"
            else:
                self.image = "run" + dir_index + str((app.game.timer // 8) % 4)
...
class Robot(GravityActor):
    ...
    def update(self):
        ...
        if self.change_dir_timer <= 0:
            ...
            directions = [-1, 1]
            if app.game.player:
                directions.append(sign(app.game.player.x - self.x))
        ...
        if self.fire_timer >= 12:
            ...
            fire_probability = app.game.fire_probability()
            if app.game.player and self.top < app.game.player.bottom and self.bottom > app.game.player.top:
                fire_probability *= 10
            if random() < fire_probability:
                self.fire_timer = 0
                app.game.play_sound("laser", 4)
            ...
            app.game.bolts.append(Bolt((self.x + self.direction_x * 20, self.y - 38), self.direction_x))
        ...
        if self.fire_timer < 12:
            ...
            image += str(1 + ((app.game.timer // 4) % 4))
        self.image = image
...
app.change_state(MenuScreen(app))
